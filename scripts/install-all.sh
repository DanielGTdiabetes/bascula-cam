...
# - Resuelve conflictos entre libcamera0 y libcamera0.5
#
...
if [[ ${EUID:-$(id -u)} -ne 0 ]]; then
  err "Ejecuta con sudo: sudo ./install-all_complete_autodetect_fixed.sh"
  exit 1
fi
...
# CÁMARA: MANEJO MEJORADO DE DEPENDENCIAS
log "Instalando paquetes de cámara con manejo mejorado de dependencias…"

# Primero, liberar todos los holds relacionados con libcamera
log "Liberando holds de paquetes libcamera…"
...
# Función para limpiar instalaciones previas problemáticas
cleanup_libcamera() {
  log "Limpiando instalaciones previas de libcamera…"
  # Remover paquetes conflictivos si existen
  for pkg in libcamera0 libcamera0.5 libcamera-ipa; do
    if dpkg -l | grep -q "^ii.*$pkg "; then
      log "Removiendo $pkg existente…"
...
# Función para intentar instalación con libcamera0.5 (serie nueva)
install_new_series() {
  log "Intentando instalación de serie NUEVA: libcamera0.5 + libcamera-ipa"
  
  # Instalar en orden específico para evitar conflictos
  if apt-get install -y --no-install-recommends libcamera-ipa; then
    if apt-get install -y --no-install-recommends libcamera0.5; then
      if apt-get install -y python3-picamera2; then
        # Intentar rpicam-apps, si falla usar libcamera-apps
        if ! apt-get install -y rpicam-apps; then
          warn "rpicam-apps no disponible; usando libcamera-apps"
          apt-get install -y libcamera-apps || return 1
        fi
        return 0
      fi
    fi
...
# Función para instalar libcamera0 (serie vieja)
install_old_series() {
  log "Instalando serie VIEJA: libcamera0"
  
  if apt-get install -y --no-install-recommends libcamera0; then
    if apt-get install -y python3-picamera2; then
      # Intentar rpicam-apps, si falla usar libcamera-apps
      if ! apt-get install -y rpicam-apps; then
        warn "rpicam-apps no disponible; usando libcamera-apps"
...
# Actualizar índices de paquetes antes de intentar instalaciones
apt-get update -y

# Verificar disponibilidad de libcamera0.5
if apt-cache policy libcamera0.5 2>/dev/null | grep -q 'Candidate:.*[0-9]'; then
  log "libcamera0.5 disponible en repositorios"
  
  # Intentar instalación directa primero
  if ! install_new_series; then
    warn "Instalación directa falló, limpiando e intentando nuevamente…"
    cleanup_libcamera
    if ! install_new_series; then
      warn "Serie nueva falló, intentando serie vieja…"
      if ! install_old_series; then
        err "Falló instalación de ambas series de libcamera"
        exit 1
      fi
    fi
  fi
else
  log "libcamera0.5 no disponible, usando serie vieja"
  if ! install_old_series; then
    warn "Serie vieja falló, limpiando e intentando nuevamente…"
    cleanup_libcamera
    if ! install_old_series; then
      err "No se pudo instalar ninguna versión de libcamera"
      exit 1
    fi
  fi
fi
...
if python3 -c "import picamera2; print('Picamera2 importado correctamente')" 2>/dev/null; then
  log "Instalación de cámara exitosa - Picamera2 funcional"
else
  warn "Picamera2 instalado pero puede requerir reinicio para funcionar correctamente"
fi
