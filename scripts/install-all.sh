...
if [[ "${SKIP_INSTALL_ALL_SERVICE_DEPLOY:-0}" != "1" ]]; then
  ...
  if [[ -f "${bascula_env_dest}" ]]; then
    ...
    done < "${bascula_env_dest}"
    if ((${#env_keys[@]})); then
      mapfile -t env_keys < <(printf '%s\n' "${env_keys[@]}" | awk '!seen[$0]++')
      for key in "${env_keys[@]}"; do
        value="$(env -i bash -c 'set -a; source "$1" >/dev/null 2>&1; key_name="$2"; if [[ -n "$key_name" && -n "${!key_name+x}" ]]; then printf %s "${!key_name}"; fi' bash "${bascula_env_dest}" "${key}")"
        env_values["${key}"]="${value}"
      done
    fi
  fi

  if [[ -n "${prev_host}" && -z "${env_values[HOST]:-}" ]]; then
    env_values[HOST]="${prev_host}"
    migrated_host=1
  fi
  if [[ -n "${prev_port}" && -z "${env_values[PORT]:-}" ]]; then
    env_values[PORT]="${prev_port}"
    migrated_port=1
  fi

  env_values[APP_MODULE]="${env_values[APP_MODULE]:-bascula.web.app:app}"
  env_values[HOST]="${env_values[HOST]:-127.0.0.1}"
  env_values[PORT]="${env_values[PORT]:-8080}"
  env_values[WORKERS]="${env_values[WORKERS]:-1}"

  tmp_env_file="$(mktemp)"
  {
    echo "# bascula-web systemd service configuration"
    echo "# Generated by scripts/install-all.sh"
    for key in APP_MODULE HOST PORT WORKERS; do
  ...
  install -D -m 0644 "${tmp_env_file}" "${bascula_env_dest}"
  rm -f "${tmp_env_file}"
  echo "[info] Updated /etc/default/bascula-web"
  if (( migrated_host )); then
    echo "[info] bascula-web HOST migrated from legacy /etc/default/bascula (${prev_host})"
  fi
  if (( migrated_port )); then
    echo "[info] bascula-web PORT migrated from legacy /etc/default/bascula (${prev_port})"
  fi
  env_updated=1
