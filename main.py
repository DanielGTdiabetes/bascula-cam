...
class NumPadDialog(tk.Toplevel):
    def __init__(self, master, title="Calibración (g)", initial=""):
        ...
        entry = tk.Entry(wrap, textvariable=self.value, font=("Arial", 36),
        ...
        tk.Button(bwrap, text="Cancelar", font=("Arial", 22),
                  width=8, command=self._cancel).pack(side=tk.LEFT, padx=8)
        tk.Button(bwrap, text="OK", font=("Arial", 22),
                  width=8, command=self._ok).pack(side=tk.LEFT, padx=8)
...
class ScaleApp:
    def __init__(self):
        ...
        outer.grid_rowconfigure(2, weight=0)  # línea mensajes
        ...
        self.status_label = tk.Label(outer, textvariable=self.stable_var,
                                     font=("Arial", 28), fg="#9acd32", bg="black")
        self.status_label.grid(row=1, column=0, pady=(0,10))

        # Mensajes (tara, calibración, errores)
        self.msg_var = tk.StringVar(value="")
        self.msg_label = tk.Label(outer, textvariable=self.msg_var,
                                  font=("Arial", 22), fg="#00BFFF", bg="black")
        self.msg_label.grid(row=2, column=0, pady=(0,10))

        # Botonera
        btns = tk.Frame(outer, bg="black")
        btns.grid(row=3, column=0, pady=(10,40))

        def mkbtn(text, cmd):
            return tk.Button(btns, text=text, font=("Arial", 26),
                             width=10, height=2, command=cmd)
    ...
    def do_tare(self):
        ok = self.scale.tare()
        self.msg_var.set("✔ Tara realizada" if ok else "✖ Error al tarar")

    def do_cal_preset(self, grams):
        ok = self.scale.calibrate(float(grams))
        self.msg_var.set(f"✔ Calibrado con {grams} g" if ok else f"✖ Error calibrando {grams} g")
    ...
    def do_cal_dialog(self):
        ...
        if dlg.result is not None:
            ...
            self.msg_var.set(
                f"✔ Calibrado con {dlg.result:g} g" if ok else f"✖ Error calibrando {dlg.result:g} g"
            )
    ...
    def update_loop(self):
        ...
        display_int = int(round(g))  # enteros

        self.weight_var.set(f"{display_int} g")
        if s:
            self.stable_var.set("Estable ✓")
            self.status_label.config(fg="#41D888")
        else:
            self.stable_var.set("Inestable …")
            self.status_label.config(fg="#FFB020")
