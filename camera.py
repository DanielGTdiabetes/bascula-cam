...
class CameraService:
    def __init__(self, prefer_backend: Optional[str] = None, resolution: Tuple[int,int]=(640,480)):
        ...
        self._loop_running = False; self._loop_after_id = None
    ...
    def attach_preview(self, tk_container):
        ...
        self._loop_running = True; self._schedule_next(30)
    ...
    def _schedule_next(self, ms):
        if not self._tk_parent: return
        try: self._loop_after_id = self._tk_parent.after(ms, self._update_preview)
        except Exception: self._loop_after_id = None

    def _update_preview(self):
        if not self._loop_running or not self._tk_label:
            return
        frame_rgb = self._grab_frame_rgb()
        if frame_rgb is not None and Image is not None:
            try:
                h = max(100, self._tk_parent.winfo_height())
                w = max(100, self._tk_parent.winfo_width())
                img = Image.fromarray(frame_rgb); img.thumbnail((w,h))
    ...
    def _grab_frame_rgb(self):
        if self.backend == "picamera2":
            try:
                arr = self.picam.capture_array()  # RGB
                return arr
            except Exception:
                return None
