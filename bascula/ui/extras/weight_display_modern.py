...
class AnimatedWeightDisplay(tk.Frame):
    def __init__(self, parent):
        ...
        card = tk.Frame(self, bg=MT.surface, bd=1, relief="solid", highlightbackground=MT.surface_light,
                        highlightcolor=MT.surface_light, highlightthickness=1)
        ...
        self.lbl = tk.Label(inner, textvariable=self.var, font=("Segoe UI", 64, "bold"),
                            fg=MT.success, bg=MT.surface)
        self.lbl.pack()

        bottom = tk.Frame(inner, bg=MT.surface); bottom.pack(fill="x", pady=(16,0))
        self.unit = tk.Label(bottom, text="GRAMOS", font=("Segoe UI", 14, "bold"),
                             fg=MT.primary, bg=MT.surface)
        self.unit.pack(side="left")
        right = tk.Frame(bottom, bg=MT.surface); right.pack(side="right")
        self.stab = tk.Label(right, text="● MIDIENDO", font=("Segoe UI", 11, "bold"),
                             fg=MT.warning, bg=MT.surface)
        self.stab.pack()
    ...
    def set(self, value: float, stable: bool, confidence: float):
        ...
        # stability
        if stable:
            self.stab.configure(text="● ESTABLE", fg=MT.success)
        else:
            self.stab.configure(text="● MIDIENDO", fg=MT.warning)
