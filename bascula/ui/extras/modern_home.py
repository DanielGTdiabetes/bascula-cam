...
class ModernHome(tk.Frame):
    def __init__(self, root, state, storage, logger, scale, camera):
        ...
        self.state = state; self.storage = storage; self.logger = logger; self.scale = scale; self.camera = camera
        header = tk.Frame(self, bg=MT.surface, bd=1, relief="solid", highlightbackground=MT.surface_light, highlightthickness=1)
        header.pack(fill="x", padx=16, pady=(16,8))
        tk.Label(header, text="⚖️ SMART SCALE PRO", font=("Segoe UI", 18, "bold"), fg=MT.text, bg=MT.surface).pack(side="left", padx=12, pady=8)
        ...
        ModernButton(btns, "🔄 TARA", lambda: self._tara(), "success", "lg", width=16).pack(side="left", padx=(0,8))
    ...
    def _tick(self):
        ...
        if abs(self.state.current_weight) > 0.5: self.stats.update(self.state.current_weight)
        self.after(200, self._tick)
    def _tara(self):
        if self.scale.tara(): pass
    def _save(self):
        from datetime import datetime
        rec = {"timestamp": datetime.now().isoformat(), "weight": round(self.state.current_weight, 1),
               "unit":"g","stable": self.scale.filter.stable}
        try: self.storage.append_measurement(rec)
        except Exception as e: self.logger.error(f"save error: {e}")
    def _reset(self): self.stats.reset()
