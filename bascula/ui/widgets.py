...
def bind_touch_scroll(widget, *, units_divisor=2):
    ...
    def _press(e):
        ...
        if hasattr(widget, "yview_scan"):
            widget.yview_scan("mark", e.x, e.y)
        # No devolvemos 'break' para permitir selección por tap
        return None
    ...
    def _drag(e):
        ...
        if hasattr(widget, "yview_scan"):
            ...
            try:
                ...
                pass
        # Solo cortamos el evento cuando realmente estamos arrastrando
        return "break" if st["drag"] else None

    def _release(e):
        # Si hubo drag, cortamos el release para evitar selección accidental
        if st["drag"]:
            st["drag"] = False
            return "break"
        return None
