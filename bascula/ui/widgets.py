...
def bind_touch_scroll(widget, *, units_divisor=2):
    """
    Scroll táctil por arrastre.
    - Canvas: usa yview_scan mark/dragto
    - Listbox/Text: yview_scroll
    - ttk.Treeview: **no se liga** (para no interferir con selección). Usa la barra de scroll.
    """
    # Si es un Treeview (tiene identify_row), no ligamos handlers: evitar conflictos de selección.
    if hasattr(widget, "identify_row"):
        return  # barra de scroll manejará el desplazamiento
