# -*- coding: utf-8 -*-
# bascula/ui/screens.py - Acciones visibles, nutrición con más espacio
...
class HomeScreen(BaseScreen):
    def __init__(self, parent, app, on_open_settings_menu):
        ...
        # Dar MÁS peso al panel de nutrición
        right.grid_rowconfigure(0, weight=2, uniform="r")
        right.grid_rowconfigure(1, weight=3, uniform="r")
        right.grid_columnconfigure(0, weight=1)

        # Estilo Treeview (rebajo rowheight para ganar vertical)
        ...
        # --- Card Lista de alimentos (grid interno: header, sep, tree, actions) ---
        self.card_items = Card(right, min_width=320, min_height=220); self.card_items.grid(row=0, column=0, sticky="nsew")
        items_root = tk.Frame(self.card_items, bg=COL_CARD); items_root.pack(fill="both", expand=True)
        items_root.grid_rowconfigure(0, weight=0)   # header
        items_root.grid_rowconfigure(1, weight=0)   # sep
        items_root.grid_rowconfigure(2, weight=1)   # tree
        items_root.grid_rowconfigure(3, weight=0)   # actions
        items_root.grid_columnconfigure(0, weight=1)

        header_items = tk.Frame(items_root, bg=COL_CARD)
        header_items.grid(row=0, column=0, sticky="ew")
        ...
        sep = tk.Frame(items_root, bg=COL_ACCENT, height=1); sep.grid(row=1, column=0, sticky="ew", pady=(4,6))

        tree_frame = tk.Frame(items_root, bg=COL_CARD)
        ...
        actions = tk.Frame(items_root, bg=COL_CARD)
        ...
        # --- Card Nutrición (más alta para asegurar 'Grasas') ---
        self.card_nutrition = Card(right, min_width=320, min_height=320)
        ...
        grid.pack(fill="both", expand=True, padx=8, pady=(6,10), anchor="n")
        self._nut_labels = {}
