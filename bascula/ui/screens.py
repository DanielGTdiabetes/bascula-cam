...
# VERSIÓN FINAL: UI avanzada con la cámara real integrada.
...
class HomeScreen(BaseScreen):
    def __init__(self, parent, app, on_open_settings_menu):
        ...
        self.status_indicator = StatusIndicator(header_weight, size=16); self.status_indicator.pack(side="left", padx=(get_scaled_size(10),0)); self.status_indicator.set_status("inactive")
        ...
        # ... (código de la tarjeta de nutrición sin cambios)
        ...
        # ... (código de la lista de alimentos sin cambios)
    ...
    def _tick(self):
        try:
            ...
            self.weight_lbl.config(text="Error"); self.status_indicator.set_status("inactive"); self.stability_label.config(text="○ Sin señal", fg=COL_MUTED)
        finally:
            self.after(80, self._tick)

    def _on_tara(self):
        raw_val = self.app.get_reader().get_latest()
        if raw_val is None:
    ...
    def _on_plato(self): self.toast.show("🍽 Función de Plato (pendiente)", 1000, COL_ACCENT)

    # --- MÉTODO MODIFICADO CON CÁMARA REAL ---
    def _on_add_item(self):
        ...
        tk.Label(cont, text="📷 Capturar Alimento", bg=COL_CARD, fg=COL_ACCENT, 
                 font=("DejaVu Sans", FS_TITLE, "bold")).pack(anchor="w")
        ...
        if self.app.camera and self.app.camera.is_available():
            ...
            tk.Label(camera_area, text="Cámara no disponible", bg="#000000", fg=COL_DANGER).pack(expand=True)
...
# ... El resto de las clases de pantalla (SettingsMenuScreen, CalibScreen, etc.)
# se mantienen exactamente igual que en el archivo que subiste, así que las incluyo
# para que el archivo esté completo.
...
class CalibScreen(BaseScreen):
    ...
    def __init__(self, parent, app):
        ...
        tk.Label(rowp, text="Peso patrón (g):", bg=COL_CARD, fg=COL_TEXT, font=("DejaVu Sans", FS_TEXT)).pack(side="left")
    ...
    def _calc_save(self):
        ...
        self.after(800, lambda:self.app.show_screen('settingsmenu'))
...
class WifiScreen(BaseScreen):
    ...
    def _scan_networks(self):
        nets = self.app.wifi_scan() or ["Intek_5G", "Intek_2G", "Casa_Dani", "Invitados"]
...
class ApiKeyScreen(BaseScreen):
    ...
    def _test_local(self):
        k = self._key_var.get().strip(); ok = len(k) > 20 and k.startswith("sk-")
        self.toast.show("✓ Formato parece correcto" if ok else "⚠ Clave sospechosa", 1100, COL_SUCCESS if ok else COL_WARN)
