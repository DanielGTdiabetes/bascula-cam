...
class HomeScreen(BaseScreen):
    ...
    def __init__(self, parent, app, on_open_settings_menu):
        ...
        for text, cmd, r, c, color in btn_map:
            b = tk.Button(btns, text=text, command=cmd, bg=color, fg="white", font=("DejaVu Sans", FS_BTN_SMALL, "bold"),
        ...
        center = Card(self)
        center.config(bg="#bbdefb")  # Fondo azul claro temporal para debug (puedes quitarlo después)
        center.grid(row=0, column=1, sticky="nsew", padx=6, pady=10)
        center.grid_rowconfigure(1, weight=1)  # Permitir expansión vertical en la lista
        ...
        right.grid_rowconfigure(1, weight=1)  # Permitir expansión vertical en consejos

        totals = Card(right)
        totals.pack(fill="x", expand=False, pady=(0, 10))

        tk.Label(totals, text="Totales", bg=COL_CARD, fg=COL_ACCENT, font=("DejaVu Sans", FS_CARD_TITLE, "bold")).pack(padx=10, pady=(10, 5))

        grid = tk.Frame(totals, bg=COL_CARD)
        ...
        for name, key, unit in [("Peso total", "grams", "g"), ("Calorías", "kcal", "kcal"),
            ...
            val.pack(side="right")
            tk.Label(row, text=f" {unit}", bg=COL_CARD, fg=COL_MUTED, font=("DejaVu Sans", FS_TEXT - 1)).pack(side="right")
    ...
    def _add_item_from_data(self, data):
        ...
        self.tree.insert("", "end", iid=str(data['id']),
                         values=(data.get('name', '?'), f"{data.get('grams', 0):.0f} g", f"{data.get('kcal', 0):.0f}"))
    ...
    def _on_finish_meal_open(self):
        ...
        rows = [("Peso total", totals['grams'], 'g'), ("Calorías", totals['kcal'], 'kcal'),
                ("Carbohidratos", totals['carbs'], 'g'), ("Proteínas", totals['protein'], 'g'),
                ("Grasas", totals['fat'], 'g')]
