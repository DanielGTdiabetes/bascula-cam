# -*- coding: utf-8 -*-
# bascula/ui/screens.py - Nutrición con grid (incluye Grasas visible), botón Borrar visible y Reiniciar
...
class HomeScreen(BaseScreen):
    def __init__(self, parent, app, on_open_settings_menu):
        ...
        # CARD PESO - Altura reducida para dar más espacio a nutrición
        self.card_weight = Card(self, min_width=700, min_height=300)  # Reducido de 400
        ...
        # Ajustando proporciones para más espacio a nutrición
        right.grid_rowconfigure(0, weight=5, uniform="r")  # Más peso a la lista
        right.grid_rowconfigure(1, weight=4, uniform="r")  # Más peso a nutrición
        ...
        self.card_items = Card(right, min_width=320, min_height=200); self.card_items.grid(row=0, column=0, sticky="nsew")
        ...
        # NUTRICIÓN - con altura mínima aumentada para asegurar visibilidad
        self.card_nutrition = Card(right, min_width=320, min_height=200)  # Altura mínima aumentada
        ...
        tk.Frame(self.card_nutrition, bg=COL_ACCENT, height=1).pack(fill="x", pady=(4,4))

        # Grid con menos padding para aprovechar espacio
        grid = tk.Frame(self.card_nutrition, bg="#1a1f2e", highlightbackground=COL_BORDER, highlightthickness=1, relief="flat")
        grid.pack(fill="both", expand=True, padx=6, pady=(4,6))  # expand=True para llenar espacio
        self._nut_labels = {}
        ...
        for r,(name,key) in enumerate(names):
            grid.grid_rowconfigure(r, weight=1)  # Peso uniforme para cada fila
            ...
            lbl.grid(row=r, column=0, sticky="w", padx=8, pady=(1,1))  # Padding reducido
