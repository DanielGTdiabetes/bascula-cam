# -*- coding: utf-8 -*-
"""
Bascula UI - MODIFICADO: Integración completa de cámara real (Picamera2).
"""
import os
import time
import random
...
# Se utiliza el backend de comunicación real con el ESP32
from python_backend.serial_scale import SerialScale
from utils import load_config, save_config

# Importación segura de la librería de la cámara
try:
...
class BasculaAppTk:
    ...
    def _init_services(self):
        try:
            ...
            if PICAMERA_AVAILABLE:
                ...
                # Configuración optimizada para preview en la pantalla y captura de alta calidad
                preview_config = self.picam2.create_preview_configuration(main={"size": (1024, 768)})
                self.picam2.configure(preview_config)
                self.picam2.start()
                print("[APP] Picamera2 inicializada correctamente.")
            else:
                print("[APP] ADVERTENCIA: Picamera2 no está disponible. Se usará la captura simulada.")

        except Exception as e:
            print(f"[APP] Error fatal inicializando servicios: {e}")
            self.cfg = {"port":"/dev/serial0","baud":115200,"unit":"g","decimals":0,"openai_api_key":""}
            self.reader = None
    
    def _update_weight_data(self, grams, stable):
        """Callback que recibe los datos procesados directamente del ESP32."""
    ...
    def _on_close(self):
        ...
        except Exception as e: print(f"[APP] Error durante el cierre: {e}")
    ...
    def start_camera_preview(self):
        if self.picam2:
            sw, sh = self.root.winfo_screenwidth(), self.root.winfo_screenheight()
            # Preview.DRM es el más eficiente para modo kiosko sin escritorio
    ...
    def capture_image(self) -> str:
        ...
        if self.picam2:
            try:
                # Cambiamos a una configuración de alta resolución para la foto
                capture_config = self.picam2.create_still_configuration()
                self.picam2.switch_mode_and_capture_file(capture_config, path)
                print(f"[APP] Imagen real capturada en {path}")
                return path
            except Exception as e:
                print(f"[APP] Error al capturar imagen: {e}")
        
        # Si la cámara falla o no está disponible, crea un archivo vacío
        with open(path, "wb") as f: f.write(b"")
        print(f"[APP] Imagen simulada creada en {path}")
        return path
