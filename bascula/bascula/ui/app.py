...
class BasculaAppTk:
        ...
        try:
            ...
            if PICAMERA_AVAILABLE:
                try:
                    ...
                    cfg = self.picam2.create_preview_configuration(main={'size': (1024, 768), 'format': 'RGB888'})
    ...
    return bool(getattr(self, "picam2", None))

def camera_attach_preview(self, tk_container):
    # Preview embebido en Tk (sin libcamera-hello)
    import tkinter as tk
    if not (self.picam2 and PIL_AVAILABLE):
        return
    self.camera_detach_preview()
    self._cam_label = tk.Label(tk_container, bg="#000000")
    self._cam_label.pack(fill="both", expand=True)
    # pequeño warm-up ya hecho con start(); programamos loop
    def _tick():
        if not self._cam_label: return
        try:
            arr = self.picam2.capture_array()  # RGB888
            if arr is not None:
                img = Image.fromarray(arr)
                w = max(100, self._cam_label.winfo_width()); h = max(100, self._cam_label.winfo_height())
                img.thumbnail((w, h))
                tkimg = ImageTk.PhotoImage(img)
                self._cam_label.configure(image=tkimg)
                self._cam_label.image = tkimg  # mantener ref
        except Exception:
            pass
        self._cam_after = self._cam_label.after(60, _tick)
    _tick()

def camera_detach_preview(self):
    if self._cam_label:
        try:
            if self._cam_after and hasattr(self._cam_label, "after_cancel"):
                try: self._cam_label.after_cancel(self._cam_after)
                except Exception: pass
        finally:
            try: self._cam_label.destroy()
            except Exception: pass
    self._cam_label = None
    self._cam_after = None
...
def capture_image(self) -> str:
    ...
    if self.picam2:
        try:
            still = self.picam2.create_still_configuration(main={'size': (2304,1296)})
