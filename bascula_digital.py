...
+ Captura de foto con Picamera2 al GUARDAR
"""
...
try:
    ...
    Picamera2  # solo para no marcar como sin usar
except Exception:
    CAM_AVAILABLE = False
...
class BasculaDigital:
    def __init__(self, root):
        ...
        self.setup_camera()     # <-- inicialización de cámara (segura)
        self.setup_hx711()
        self.create_widgets()
        self.start_reading()
    ...
    def setup_camera(self):
        """Inicializar cámara de forma segura (si está disponible)"""
        ...
        try:
            self.cam = Picamera2()
            # Configuración sencilla para foto fija
            self.cam.configure(self.cam.create_still_configuration())
            self.cam.start()
            time.sleep(0.2)  # pequeño settle
    ...
    def create_widgets(self):
        ...
        self.camera_label = tk.Label(self.root, text=cam_state,
                                     font=('Arial', 10),
    ...
    def _capture_photo(self, weight_g: float) -> str:
        """
        Captura una foto si la cámara está lista.
        Devuelve la ruta del archivo o cadena vacía si falla.
        """
    ...
    def save_reading(self):
        ...
        if hasattr(self, 'current_weight'):
            ...
            # Intentar capturar foto (no bloqueante para el guardado)
            photo_path = ""
            if self.camera_ready:
                photo_path = self._capture_photo(self.current_weight)
                if photo_path:
            ...
            try:
                ...
                if photo_path:
                    ...
                    if self.camera_ready:
                        # cámara lista pero no devolvió path (fallo puntual)
